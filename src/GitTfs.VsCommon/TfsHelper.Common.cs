...
namespace GitTfs.VsCommon
{
    public abstract class TfsHelperBase : ITfsHelper
    {
        ...
        private int GetRelevantChangesetBasedOnChangeType(IEnumerable<MergeInfo> merges, string tfsPathParentBranch, string tfsPathBranchToCreate, out string renameFromBranch)
        {
            ...
            var mergeC = merges.Where(m => m.SourceItem.Equals(tfsPathParentBranch, StringComparison.InvariantCultureIgnoreCase)
                && !m.TargetItem.Equals(tfsPathParentBranch, StringComparison.InvariantCultureIgnoreCase)).ToArray();
            var merge = mergeC.FirstOrDefault(m => m.TargetItem.Equals(tfsPathBranchToCreate, StringComparison.InvariantCultureIgnoreCase)) ?? mergeC.FirstOrDefault();
        ...
        private static bool AddNewRootBranch(IList<RootBranch> rootBranches, RootBranch rootBranch)
        {
            ...
            Trace.TraceInformation($"Root branch: {rootBranch.DebuggerDisplay}");
            ...
            int firstChangesetInBranchToCreate, int lastChangesetIdToCheck)
        {
            ...
            var merges = VersionControl
                .TrackMerges(new int[] { firstChangesetInBranchToCreate },
                    ...
                    null)
                .OrderByDescending(x => x.SourceChangeset.ChangesetId).ToArray();
